    ...
    val allTools: List<Map<String, Any>> by lazy {
        listOf(
            ...
            tool(
                ...
                properties = mapOf(
                    "bookUrl" to prop("string", "书籍唯一标识 URL。可直接从系统提示词的【当前阅读书籍信息】中读取；否则从 get_bookshelf 获取"),
                    "notes" to propArray(
                        "感想列表，每条对应一个章节。支持一次写多个章节。",
                        mapOf(
                            "chapterIndex" to prop("integer", "章节索引，从 0 开始。可从系统提示词的【书籍目录】区块确认对应章节的序号"),
                            "selectedText" to prop("string", "关联的原文片段。需先调用 get_book_content 获取该章正文后摘取；留空则自动取章节完整内容"),
